...
class QuantitativeAnalyzer:
    ...
    def calc_speed_score(self, race_history: list[dict], moisture: float = 0, horse_name: str = "Unknown", date: str = "", gate_no: int = 0, scraper=None, meet_code: str = "1", track_bias: dict = None, race_context: dict = None, market_odds: float = 0.0, market_rank: int = 0, **kwargs) -> dict:
        """
        ...
        # [P6] 오버페이스(Overpace) 페널티 — 거리별 임계값 적용
        # 구버전: 전 거리 고정값 0.4s → 거리별 임계값으로 교체
        # 장거리(2000m+)는 0.25s 이상이면 이미 심각한 오버페이스
        overpace_penalty = 0
        if len(s1f_norm_vals) >= 2:
            _rc_dist_op = self._dist_from_race(recent)
            # 거리별 오버페이스 감지 임계값 (짧을수록 허용 범위 큼)
            _op_thresh = (
                0.40 if _rc_dist_op <= 1200 else
                0.35 if _rc_dist_op <= 1600 else
                0.30 if _rc_dist_op <= 1800 else
                0.25  # 2000m 이상 — 소폭 페이스 이탈도 과소모로 간주
            )
            if s1f_norm_vals[0] < (np.mean(s1f_norm_vals[1:]) - _op_thresh):
                if g1f_vector in ["Weak", "Declining"]: overpace_penalty = 15

        # [P2] 주로 바이어스 동적 계산 (날짜 하드코딩 제거)
        if track_bias is None:
            track_bias = TrackDynamics.quantify_track_bias(moisture, meet_code, date=date, scraper=scraper)

        # [P2] w_s1f / w_g1f — moisture + 실시간 bias 기반 동적 결정
        # 구버전: is_april(날짜 하드코딩) → 특정 시즌 과적합 위험 (삭제됨)
        # 신버전: 함수율(moisture) + 당일 각질 바이어스(front_bonus)만으로 결정
        _front_bonus = track_bias.get("front_bonus", 0)
        if 10 <= moisture <= 16:
            # 다습 주로 → 선행마 극유리 → S1F 비중 상향
            w_s1f, w_g1f = (0.65, 0.35)
        elif moisture < 6 or moisture >= 20:
            # 건조/불량 주로 → 추입마 유리 → G1F 비중 상향
            w_s1f, w_g1f = (0.35, 0.65)
        else:
            # 양호 주로 (6~9%, 17~19%) → 기본 5:5
            w_s1f, w_g1f = (0.5, 0.5)
        # 실시간 당일 바이어스가 선행 강세면 S1F 비중 추가 보정 (+5%p, 상한 0.75)
        if _front_bonus >= 10:
            w_s1f = min(0.75, w_s1f + 0.05)
    ...
    def analyze_horse(self, horse_name: str,
        ...
        speed = self.calc_speed_score(race_history, moisture=moisture, horse_name=horse_name, date=date, gate_no=gate_no, scraper=scraper, meet_code=meet_code, track_bias=track_bias, race_context=race_context, market_odds=market_odds, market_rank=market_rank, **kwargs)
    ...
    def rank_horses(self, analyses: list[dict], meet_code: str = "1", entries_with_odds: list[dict] = None, dist: int = 0, grade: str = "") -> dict:
        ...
        confidence_score = 0
        if len(valid) >= 2:
            gap = valid[0].get("total_score", 0) - valid[1].get("total_score", 0)
            base_score = valid[0].get("total_score", 0)
            if gap >= 1.5 or base_score >= 8.5: confidence_score = 100
            elif gap >= 0.8: confidence_score = 70
            elif gap >= 0.3: confidence_score = 40
            else: confidence_score = 10
            
        # [SNIPER v7.3] 가치 지수(Value Rank Sum)
        top5_market_ranks = [h.get("market_rank", 10) for h in valid[:5]]
        rank_sum = sum(top5_market_ranks)

        all_ranked = valid + vetoed
        target_info = self.classify_advanced_target(valid)
        
        # [SNIPER v7.3] 데이터 파이프라인 직결
        return self.evaluate_strategy(
            all_ranked, meet_code=meet_code, pace_flag=pace_flag, 
    ...
    def evaluate_strategy(self, ranked: list[dict], meet_code: str = "1", pace_flag: str = "", confusion_flag: str = "", target_info: dict = None, dist: int = 0, grade: str = "", confidence: int = 0, rank_sum: int = 0, n_fast_starters: int = 0) -> dict:
        ...
        # [SNI v7.3] 고배당 가치마 판별 (Radar 전용)
        # AI 예상 상위 3두 중에 배당이 15배 이상인 '숨은 보석'이 있는지 확인
        has_lotto_value = any(h.get("rank") <= 3 and h.get("market_odds", 0) >= 15.0 for h in ranked[:12])
        
        # [SNI v7.3] 종합 베팅 등급 (Sniper Grade v7.3 - Golden Lock)
        sniper_grade = "관망 - 전략적 휴식 (기회 희박)"
        bet_units = 0
        bet_guide = "💡 정량 분석 결과, 안정적인 베팅 구간이 아닙니다. 다음 경주를 기약하세요."
        
        # 1. 선행 붕괴 (SNIPER): 선행 4두 이상 시 무조건 고배당 사냥
        if n_fast_starters >= 4:
            sniper_grade = "스나이퍼 - 선행 자멸 (고배당 특화)"
            bet_units = 10
            bet_guide = "🔥 선행마 자멸 구간! 추입/복병마의 기습이 예상됩니다. 고배당 스나이핑 모드 작동."
        # 2. 고확신 승부 (SNI-PRO): 확신도 100 + 가치 24 이상
        elif confidence >= 100:
            if rank_sum >= 24:
                sniper_grade = "프로승부 - AI 압도적 우위 (고신뢰)"
                bet_units = 10
                bet_guide = "🎯 AI가 강력하게 추천하는 경주입니다. 메인 승부 구간으로 활용하세요."
            else:
                sniper_grade = "관망 - 가치 부족 고확신 (Skip)"
                bet_units = 0
        # 3. 로또형 정밀 타격 (LOTTO): 확신도 70 + 고배당마 포착
        elif has_lotto_value and confidence >= 70:
            sniper_grade = "로또 - 고배당 정밀 타격 (대물 사냥)"
            bet_units = 10
            bet_guide = "🎁 숨은 고배당마가 포착되었습니다. 저평가된 복병마를 중심으로 대박을 노리세요."
        # 4. 정예 중의 정예 (ACTIVE): 확신도 90 + 가치 30 이상
        elif rank_sum >= 30 and confidence >= 90:
        ...
        bet_mode = "NONE"
        if bet_units > 0:
            if "로또" in sniper_grade:
                bet_mode = "BOX_5" # 5두 박스 (고배당 혼전)
                bet_guide += " [v7.4 박스 베팅 적용]"
            elif "스나이퍼" in sniper_grade or "프로승부" in sniper_grade:
                bet_mode = "DUAL_AXIS" # 1,2위 쌍축 (안전장치)
                bet_guide += " [v7.4 쌍축 베팅 적용]"
            else:
                bet_mode = "SINGLE_AXIS" # 1위 고정 (효율 중시)
                bet_guide += " [v7.4 단축 베팅 적용]"

        # 감사(Audit) 데이터 패키징 (블라인드 테스트 및 검증용)
        audit_data = {
            "rank_sum": rank_sum,
            "confidence": confidence,
            "has_lotto": has_lotto_value,
            "n_fast_starters": n_fast_starters,
            "is_collapse": (n_fast_starters >= 4),
            "version": "v7.3_Golden_Weights"
        }
        ...
        return {
            ...
            "strategy_badge":  sniper_grade,
            "bet_guide":       bet_guide,
            "tactical_picks":  tactical_picks,
            "has_strong_axis": (bet_units > 0),
            "is_dual_anchor":  (confidence >= 100),
            "is_golden_value": (rank_sum >= 30),
            "avg_top3":        avg_top3,
            "sniper_grade":    sniper_grade,
            "bet_units":       bet_units,
            "bet_mode":        bet_mode,
