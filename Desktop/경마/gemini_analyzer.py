...
[너의 정체성과 분석 원칙]
1. **독립적 정성 분석자**: 파이썬 모델이 [기록/부중/기수]라는 정량적 데이터를 처리한다면, 너는 **[심판리포트/전개 사연/마명 특이성/주행 습성]**이라는 정성적 데이터를 처리한다.
2. **반골 기질(Contrarian)**: 파이썬 TOP 5 마번에 맹목적으로 동조하지 마라. 파이썬이 놓친 '심판리포트상의 불운'이나 '습성 극복 가능성'을 근거로 소신껏 **독자적인 축마와 복병**을 찾아내라.
...
5. **고도화 섹셔널 지표 활용**: 새로 추가된 정량 지표들을 적극 활용하라.
   - **Burst Index**: 막판 200m 가속도. 양수가 클수록 라스트 탄력이 강력함.
   - **Relative S1F**: 필드 평균 대비 초반 스피드 차이. (+)이면 남들보다 빠름.
   - **Recovery Rate**: 초반에 밀려도(7위 밖) 끝내 올라오는 '회복 탄력성' 확률.
   - **Leading Rate**: 선행 시도 시 입상까지 연결되는 '선행 유지력' 확률.

[분석 지시사항]
- **심판리포트(Steward Reports)**를 샅샅이 뒤져라. 진로 방해, 주행 중지, 착지 불량, 외곽 주행 등 '억울한 사연'이 있는 마필이 이번에 반등할 조건을 갖췄는지(안쪽 게이트 배정 등) 확인하라.
- **파이썬 결과와의 대조**: 파이썬의 TOP 5와 너의 추천이 다르다면, 왜 다른지(정성적 근거)를 전문가적으로 설명하라. 이것이 사용자가 너의 의견을 듣는 이유다.
...
[출력 형식 - JSON 필수로 준수]
{
    ...
    "strong_leader": [
        ...
        // [필독] 만약 완벽한 강선축마 조건에 부합하는 말이 없더라도, 출전마 중 가장 입상 가능성이 높은 선행/선입마를 '대안 축마'로 반드시 1두 선정하여 기술하라.
        // 절대로 빈 리스트([])로 남기지 마라. 사용자는 너의 '소신 있는 선택'을 원한다.
        {"horse": "[마번] 마명", "reason": "선행/선입 각질 근거 + 지구력 및 반등 포인트 명시"}
    ],
    "dark_horses": [
...
class GeminiAnalyzer:
        ...
        try:
            ...
            def _is_match(db_name: str, race_names: list) -> bool:
                ...
                for rn in race_names:
                    ...
                    if dn == rn_norm:
                        return True
                    # 한쪽이 다른 쪽을 포함 (공백 제거 후)
                    if dn in rn_norm or rn_norm in dn:
                        return True
                    # 4글자 이상 공통 접두어 (이름 앞부분 매칭)
                    prefix_len = min(4, len(dn), len(rn_norm))
                    if prefix_len >= 4 and dn[:prefix_len] == rn_norm[:prefix_len]:
                        return True
                return False
    ...
    def _format_quantitative(self, race_no: int, data: list[dict], meet_code: str = "") -> str:
        ...
        lines = [
            ...
            "(위 마번은 실제 배팅용 '출전 번호'입니다. 너는 이들과 겹치지 않는 복병이나, 이들 중 위험 요소를 찾아라.)",
            "---",
        ...
        for h in data_copy:
            h['market_odds'] = float(h.get('market_odds') or h.get('odds', 99.0))
        ...
        candidates = sorted(data_copy, key=lambda x: x.get('total_score', 0), reverse=True)[:10]
